...
- [ ] **Mobile polish pass** — go through every view at mobile widths. Views: Feed (+ All work/Following tabs — now in the `.feedcontrols` space-between row, CHECK it wraps cleanly on mobile — sort row, boost meter, engagement buttons already have `min-height:44px` tap targets under a breakpoint), Story (support rail, chapters, conversation), Explore (filter groups — category/track/status/show/**funding** chips + sort select, 5 rows now, can wrap awkwardly), Search (results, sections), Profile (hero, inline followers·following line, stats grid already 2-col under 600px), the notification bell panel, Compose (+ the permanence checkbox row), modals (Donate). Watch: chip rows overflowing, the crowded top bar (demo switcher + netchip + search + bell + avatar), grids not reflowing. NOTE: nav is `display:none` under 980px (mobile uses the bottom `.tabbar`) — the nav sliding indicator + tap-to-top on the desktop nav are desktop-only; VERIFY the mobile `.tabbar` tap-to-top behaves (it calls `tabTap` → `go`, so same-view tap should also zip to top).
- [ ] **NEXT SESSION — "Save proposal for later" (draft save)** — let someone mid-way through Compose set the proposal aside and resume later. Design decisions to make at the start (leaning noted): (1) SINGLE draft per account to start (most compose one at a time; simpler than multiple named drafts — can expand later). (2) Store in the DATABASE (Supabase), NOT local/in-memory — a draft is only useful if it survives closing the tab / switching device; in-memory would be lost on refresh (and artifacts can't use localStorage anyway). Likely a `proposal_drafts` table keyed by account_id holding the in-progress fields as JSON, OR a single JSONB `draft` column on the accounts row. RLS: owner-only (unlike proposals which are public) — a draft is PRIVATE until posted. (3) What to save = the existing store `draft*` state that already drives Compose (`draftCategory`, `draftFundingMode`, `draftMilestones`, title/summary/story, staged docs are trickier — files may need to re-attach, or save without them for v1). (4) UX: a "Save for later" button in Compose near Post; an indicator that a saved draft exists (badge on Post nav item, or a "Resume draft" prompt when opening Compose); opening Compose with a saved draft repopulates fields; clear the draft on successful post; a "discard draft" option. PHILOSOPHY FIT: drafts are private + mutable + deletable (NOT part of the permanent public record) — the clean flip side of the permanence acknowledgment we just added. Drafts = "not yet committing to the record"; posting = the permanent commit. Good framing to carry into the UX copy.
