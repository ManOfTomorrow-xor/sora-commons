...
- [x] **Feed/Explore scroll + pagination persistence** [DONE] — returning to the feed from a proposal now lands you exactly where you left off, with loaded cards intact. TWO parts: (1) SCROLL — store holds `feedScrollY`/`exploreScrollY`; captured in `open()`/`openComments()` at CLICK time (NOT `onBeforeUnmount` — too late, because App.vue's `go()` runs `window.scrollTo(0,0)` synchronously right after `active.value = view`, so the scroll is already 0 by the time the unmount hook fires); restored in `onMounted` via `nextTick(() => requestAnimationFrame(() => window.scrollTo(0, ...)))` (rAF wins over go's scrollTo because it's deferred, and the cards render synchronously from the store so the page is tall enough). The window scrolls (confirmed `window.scrollY` === `document.scrollingElement.scrollTop`), not an inner container. (2) PAGINATION — moved feed `shownCount` into the store as `feedShownCount` (default 10) so "Load more" survives Feed's unmount/remount on nav; `loadMore` increments it, `watch(sort)`/`watch(tab)` still reset it to PAGE_SIZE. Explore doesn't paginate so only scroll applies there. GOTCHA during the edit: the `commons.feedScrollY = window.scrollY;` insertion accidentally overwrote the `function openComments(p:any) {` declaration line — restored it.
- [ ] **Mobile polish pass** — go through every view at mobile widths. Views: Feed (+ All work/Following tabs — now in the `.feedcontrols` space-between row, CHECK it wraps cleanly on mobile — sort row, boost meter, engagement buttons already have `min-height:44px` tap targets under a breakpoint), Story (support rail, chapters, conversation), Explore (filter groups — category/track/status/show/**funding** chips + sort select, 5 rows now, can wrap awkwardly), Search (results, sections), Profile (hero, inline followers·following line, stats grid already 2-col under 600px), the notification bell panel, Compose (+ the new permanence checkbox row), modals (Donate). Watch: chip rows overflowing, the crowded top bar (demo switcher + netchip + search + bell + avatar), grids not reflowing. NOTE: nav is `display:none` under 980px (mobile uses the bottom `.tabbar`), so the nav sliding indicator is desktop-only — fine.
